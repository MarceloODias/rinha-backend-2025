...
// ---- slowest-requests capture (global, covers ALL http calls) ----
const TOP_SLOW_K = parseInt(__ENV.TOP_SLOW_K ?? "1000"); // increase if you want
const slowest = []; // { dur, url, method, status, expected, ts, cid }

function keepTopK(sample) {
    slowest.push(sample);
    if (slowest.length > TOP_SLOW_K) {
        slowest.sort((a, b) => b.dur - a.dur); // slowest first
        slowest.length = TOP_SLOW_K;
    }
}
...
function recordResponse(res) {
    ...
    // try to pull correlationId from payloads you send in /payments
    const cid =
        (req.body && typeof req.body === "string" && req.body.includes("correlationId"))
            ? (() => {
                try {
    ...
    keepTopK({
        ...
        status: res.status,
        expected: res.status >= 200 && res.status < 400, // mirrors expected_response:true
        ts: Date.now(),
        cid,
    });
}

// monkey-patch k6/http so ALL calls get recorded
const _get = http.get;
...
const _batch = http.batch;

http.get = function (...args) { const r = _get.apply(this, args); recordResponse(r); return r; };
http.post = function (...args) { const r = _post.apply(this, args); recordResponse(r); return r; };
http.put = function (...args) { const r = _put.apply(this, args); recordResponse(r); return r; };
http.del = function (...args) { const r = _del.apply(this, args); recordResponse(r); return r; };
http.patch = function (...args) { const r = _patch.apply(this, args); recordResponse(r); return r; };
http.head = function (...args) { const r = _head.apply(this, args); recordResponse(r); return r; };
http.options = function (...args) { const r = _options.apply(this, args); recordResponse(r); return r; };
http.request = function (...args) { const r = _request.apply(this, args); recordResponse(r); return r; };
http.batch = function (...args) {
    const out = _batch.apply(this, args);
    // batch can return Array<Response> or Object<string, Response>
    if (Array.isArray(out)) out.forEach(recordResponse);
    else if (out && typeof out === "object") Object.values(out).forEach(recordResponse);
    return out;
};
